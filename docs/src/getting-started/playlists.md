...
Playlists are YAML files stored in a `playlists/` directory (configurable via `playlists_dir`
in `mtrack.yaml`). Each playlist is named after its filename stem — `my_setlist.yaml` becomes
the playlist "my_setlist".
