...
"#;
    ...
    fn test_complete_integration_flow() {
        ...
        if song_names.contains(&"DSL Light Show Song".to_string()) {
            ...
            // Verify song has lighting shows
            assert!(dsl_song.lighting_shows().is_some());
            let lighting_shows = dsl_song.lighting_shows().unwrap();

            // Parse each lighting show
            for lighting_show in lighting_shows {
                let show_path = examples_dir()
                    .join("songs/dsl-light-show-song")
                    .join(lighting_show.file());
                let content =
